...
class MafiaClient:
        ...
        try:
            ...
            for update in stub.Follow(mafia_pb2.FollowRequest(Username = self.username, SessionId = self.session_id)):
                ...
                with self.lock:
                    ...
                    elif update.Event == mafia_pb2.GameEvent.ChatMessage:
                        self.chat_msg(update.Username, update.Message)
    ...
    def join_game(self, username):
        ...
        if response.Error != "":
            ...
            self.chat = chat.ChatClient(self.session_id)
    ...
    def publish_message(self, command):
        ...
        if len(command) <= 1:
            print('Empty message')
            return
        self.chat.send_message(self.username, ' '.join(command[1:]), str(self.session_id))
